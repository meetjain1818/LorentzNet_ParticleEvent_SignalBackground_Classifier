...
class LGEBConv(MessagePassing):
    def __init__(self,
        ...
        # MLP for edge message Φe: (2*h_dim + 2 scalars) -> hidden -> hidden
        # Inputs to Φe: hi, hj, ψ(||xi - xj||^2), ψ((xi, xj))
        # The Minkowski products are scalars.
        self.phi_e = nn.Sequential(
            nn.Linear(2 * scalar_in_channels + 2, edge_mlp_hidden_channels),
            nn.ReLU(),
    ...
    def forward(self, x_coords, h_scalars, edge_index):
        """
    ...
    def message(self, h_scalar_i, h_scalar_j, x_coord_i, x_coord_j, mode: str):
        """
